...
impl Term {
  pub fn eta_reduction(&mut self) {
    match self {
      Term::Lam { tag: t1, nam: Some(lam_var), bod } => {
        bod.eta_reduction();
        match bod.as_mut() {
          Term::App { tag: t2, fun, arg: box Term::Var { nam: var_nam } }
            if t1 == t2 && lam_var == var_nam =>
          {
      ...
      Term::Match { scrutinee, arms } => {
        ...
        for (rule, term) in arms {
          ...
          if let RulePat::Num(MatchNum::Succ(nam)) = rule {
            let mut lam =
              Term::Lam { tag: None, nam: nam.take(), bod: Box::new(std::mem::replace(term, Term::Era)) };
            lam.eta_reduction();
            match lam {
              Term::Lam { tag: None, nam: nam2, bod } => {
                *nam = nam2;
                *term = *bod;
              }
              body => {
